...
<FL l="Ημ/νία Έναρξης"><input type="text" value={form.startDate?fmtDate(form.startDate):""} placeholder="ΗΗ/ΜΜ/ΕΕ" onFocus={e=>{if(form.startDate){const p=form.startDate.split("-");e.target.value=`${p[2]}/${p[1]}/${p[0]}`;}}} onChange={e=>{let v=e.target.value.replace(/[^\d/]/g,"").slice(0,10);e.target.value=v;const parts=v.split("/");if(parts.length===3&&parts[2].length>=4){const[dd,mm,yyyy]=parts;const iso=`${yyyy}-${mm.padStart(2,"0")}-${dd.padStart(2,"0")}`;const testD=new Date(iso);if(!isNaN(testD)&&testD.toISOString().slice(0,10)===iso){s("startDate",iso);if(form.duration){const d=new Date(iso);d.setMonth(d.getMonth()+parseInt(form.duration));s("endDate",d.toISOString().slice(0,10));}}}}} style={iS}/></FL>
...
<FL l="Ημ/νία Λήξης"><div style={{...iS,background:"#FFEBEE",fontWeight:700,color:"#C62828",display:"flex",alignItems:"center",minHeight:34,borderRadius:6,padding:"0 10px"}}>{form.endDate?fmtDate(form.endDate):"—"}</div></FL>
</div>
