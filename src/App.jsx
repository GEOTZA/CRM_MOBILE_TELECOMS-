...
// PDF & A5 exports
const expPDF=(r,prov)=>{const p=PROVIDERS[prov],s=ST[r.status]||{l:"—",c:"#999",bg:"#F5F5F5",i:"?"};const f=(l,v)=>'<div style="margin-bottom:3px"><span style="font-size:0.65rem;color:#999;text-transform:uppercase;font-weight:600;display:block">'+l+'</span><span style="font-size:0.84rem;font-weight:500">'+(v||"—")+'</span></div>';const linesHtml=(r.lines&&r.lines.length>0)?r.lines.map(function(ln,i){var title=ln.type==="mobile"?"📱 Κινητή":"📞 Σταθερή";var fields=[["Πρόγραμμα",ln.prog],["Τιμή","€"+(ln.price||"0")],["Τρόπος",ln.mode==="simo"?"SIM Only":"Επιδότηση"]];if(ln.simCard)fields.push(["Κάρτα SIM",ln.simCard]);if(ln.mobNum)fields.push(["Αριθμός",ln.mobNum]);if(ln.instAddr)fields.push(["Εγκατάσταση",ln.instAddr+" "+ln.instCity]);return '<div class="sc"><div class="st">'+title+" #"+(i+1)+'</div><div class="g">'+fields.map(function(x){return f(x[0],x[1]);}).join("")+'</div></div>';}).join(""):"";const datesHtml=(r.startDate||r.duration)?'<div class="sc"><div class="st">📅 Συμβόλαιο</div><div class="g">'+f("Διάρκεια",(r.duration||"24")+" μήνες")+f("Έναρξη",r.startDate||"--")+f("Λήξη",r.endDate||"--")+'</div></div>':"";const html='<!DOCTYPE html><html><head><meta charset="UTF-8"><title>'+r.id+'</title><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:Arial,sans-serif;padding:20px;max-width:800px;margin:auto;color:#222}.h{background:'+p.color+';color:#fff;padding:14px;border-radius:6px;margin-bottom:14px;display:flex;justify-content:space-between;align-items:center}.h h1{font-size:1.1rem}.bd{padding:3px 8px;border-radius:4px;font-size:0.76rem;font-weight:700;background:'+s.bg+';color:'+s.c+'}.sc{border:1px solid #E0E0E0;border-radius:5px;padding:12px;margin-bottom:10px}.st{font-weight:700;font-size:0.88rem;margin-bottom:8px;border-bottom:2px solid '+p.color+';padding-bottom:3px}.g{display:grid;grid-template-columns:repeat(3,1fr);gap:5px}.sig{text-align:center;padding:10px;border:1px solid #DDD;border-radius:6px}.sig img{max-width:260px}@media print{@page{margin:1cm}}</style></head><body><div class="h"><h1>'+p.icon+' '+r.id+' — '+p.name+'</h1><span class="bd">'+s.i+' '+s.l+'</span></div><div class="sc"><div class="st">👤 Πελάτης</div><div class="g">'+[["Επώνυμο",r.ln],["Όνομα",r.fn],["ΑΔΤ",r.adt],["Κινητό",r.mob],["ΑΦΜ",r.afm],["Email",r.em],["Διεύθυνση",r.addr],["Πόλη",r.city],["ΤΚ",r.tk]].map(function(x){return f(x[0],x[1]);}).join("")+'</div></div><div class="sc"><div class="st">📱 Πρόγραμμα</div><div class="g">'+[["Υπηρεσία",r.svc],["Πρόγραμμα",r.prog],["Τύπος",r.lt],["Τιμή","€"+r.price],["Agent",r.agentName],["Partner",r.partner]].map(function(x){return f(x[0],x[1]);}).join("")+'</div></div>'+linesHtml+datesHtml+(r.comments&&r.comments.length?'<div class="sc"><div class="st">💬 Σχόλια</div>'+r.comments.map(function(c){return '<div style="padding:3px 0;border-bottom:1px solid #F0F0F0;font-size:0.78rem"><strong>'+c.uname+'</strong> <span style="color:#999;font-size:0.68rem">'+c.ts+'</span><br/>'+c.text+'</div>';}).join("")+'</div>':"")+'<div class="sc"><div class="st">✍️ Υπογραφή</div><div class="sig">'+(r.sig?'<img src="'+r.sig+'"/>':"—")+'</div></div><script>window.onload=function(){window.print()}</script></body></html>';const w=window.open("","_blank");w.document.write(html);w.document.close();};
...
if(srch.dateFrom)res=res.filter(r=>r.created>=srch.dateFrom);
if(srch.dateTo)res=res.filter(r=>r.created<=srch.dateTo+"T23:59");
if(srch.actFrom)res=res.filter(r=>r.startDate&&r.startDate>=srch.actFrom);
if(srch.actTo)res=res.filter(r=>r.startDate&&r.startDate<=srch.actTo);
...
<div><div style={fL}>Ημ/νία Καταχ. Από</div><input type="date" value={srch.dateFrom} onChange={ss("dateFrom")} style={iS}/></div>
<div><div style={fL}>Ημ/νία Καταχ. Έως</div><input type="date" value={srch.dateTo} onChange={ss("dateTo")} style={iS}/></div>
<div><div style={fL}>Ημ/νία Ενεργ. Από</div><input type="date" value={srch.actFrom||""} onChange={ss("actFrom")} style={iS}/></div>
<div><div style={fL}>Ημ/νία Ενεργ. Έως</div><input type="date" value={srch.actTo||""} onChange={ss("actTo")} style={iS}/></div>
...
<button onClick={()=>{const h=["ID","Επώνυμο","Όνομα","ΑΦΜ","Κινητό","Πάροχος","Πρόγραμμα","Κατάσταση","Agent","Ημ/νία Καταχ.","Διάρκεια","Ημ/νία Έναρξης","Ημ/νία Λήξης"];const rows=res.map(r=>{const el=(r.lines||[])[0]||{};return[r.id,r.ln,r.fn,r.afm,r.mob,ENERGY_PROVIDERS[el.eProv]?.name||"",el.progCustom||el.prog||"",ST[r.status]?.l||"",r.agentName,fmtDate(r.created),r.duration?r.duration+" μήνες":"--",fmtDate(r.startDate),fmtDate(r.endDate)];});expReport("Αναζήτηση Ρεύματος",h,rows,"Energy_Search.xlsx");}} style={{padding:"6px 14px",borderRadius:6,border:"1px solid #4CAF50",background:"#E8F5E9",color:"#2E7D32",cursor:"pointer",fontWeight:600,fontSize:"0.78rem"}}>📥 Excel</button>
</div>
...
<thead><tr style={{background:"#FF6F00",color:"white"}}>{["ID","Πελάτης","ΑΦΜ","Πάροχος","Κατάσταση","Agent","Καταχ.","Έναρξη","Λήξη"].map(h=><th key={h} style={{padding:"7px 8px",textAlign:"left"}}>{h}</th>)}</tr></thead>
...
const EnergyForm=({ed,onCancel})=>{
  ...
  <div style={{fontFamily:"'Outfit'",fontWeight:700,fontSize:"0.88rem",marginBottom:8}}>📅 Διάρκεια Συμβολαίου</div>
  {!form.startDate&&<div style={{background:"#FFF3E0",borderRadius:6,padding:8,marginBottom:8,fontSize:"0.76rem",color:"#E65100",fontWeight:600}}>⏳ Η ημ/νία έναρξης θα οριστεί αυτόματα κατά την ενεργοποίηση</div>}
  <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))",gap:8}}>
  <FL l="Διάρκεια (μήνες)"><select value={form.duration||"24"} onChange={e=>{const dur=e.target.value;s("duration",dur);if(form.startDate&&dur){const d=new Date(form.startDate);d.setMonth(d.getMonth()+parseInt(dur));s("endDate",d.toISOString().slice(0,10));}}} style={iS}><option value="6">6 μήνες</option><option value="12">12 μήνες</option><option value="18">18 μήνες</option><option value="24">24 μήνες</option></select></FL>
  <FL l="Ημ/νία Έναρξης"><div style={{...iS,background:"#F5F5F5",display:"flex",alignItems:"center",minHeight:34,color:form.startDate?"#333":"#999"}}>{form.startDate?fmtDateFull(form.startDate):"Αυτόματα στην ενεργοποίηση"}</div></FL>
  <FL l="Ημ/νία Λήξης"><div style={{...iS,background:form.endDate?"#FFEBEE":"#F5F5F5",fontWeight:700,color:form.endDate?"#C62828":"#999",display:"flex",alignItems:"center",minHeight:34}}>{form.endDate?fmtDateFull(form.endDate):"--"}</div></FL>
  </div>
  </div>
...
{(r.startDate||r.endDate||r.duration)&&<div style={{background:"white",borderRadius:10,padding:14,marginBottom:10,border:"1px solid #E0E0E0",borderLeft:"4px solid #2E7D32"}}>
<div style={{fontFamily:"'Outfit'",fontWeight:700,fontSize:"0.88rem",marginBottom:8}}>📅 Διάρκεια Συμβολαίου</div>
<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))",gap:6}}>
<div><div style={{fontSize:"0.65rem",color:"#888",fontWeight:600,textTransform:"uppercase"}}>Διάρκεια</div><div style={{fontSize:"0.82rem",fontWeight:500}}>{r.duration?r.duration+" μήνες":"--"}</div></div>
<div><div style={{fontSize:"0.65rem",color:"#888",fontWeight:600,textTransform:"uppercase"}}>Ημ/νία Έναρξης</div><div style={{fontSize:"0.82rem",fontWeight:500}}>{fmtDate(r.startDate)}</div></div>
<div><div style={{fontSize:"0.65rem",color:"#888",fontWeight:600,textTransform:"uppercase"}}>Ημ/νία Λήξης</div><div style={{fontSize:"0.82rem",fontWeight:500}}>{fmtDate(r.endDate)}</div></div>
{r.creditDate&&<div><div style={{fontSize:"0.65rem",color:"#888",fontWeight:600,textTransform:"uppercase"}}>Ημ/νία Πίστωσης</div><div style={{fontSize:"0.82rem",fontWeight:500}}>{fmtDate(r.creditDate)}</div></div>}
</div></div>}
...
<select value={r.status} onChange={async e=>{const ns=e.target.value;const upd={status:ns};if(ns==="active"&&!r.startDate){const td2=new Date().toISOString().slice(0,10);const dur2=parseInt(r.duration)||24;const ed2=new Date(td2);ed2.setMonth(ed2.getMonth()+dur2);upd.startDate=td2;upd.start_date=td2;upd.endDate=ed2.toISOString().slice(0,10);upd.end_date=ed2.toISOString().slice(0,10);}if(ns==="credited"){upd.creditDate=new Date().toISOString().slice(0,10);upd.credit_date=new Date().toISOString().slice(0,10);}setReqs(p=>p.map(x=>x.id===r.id?{...x,...upd}:x));setESel(p=>({...p,...upd}));if(USE_SUPA){try{const dbU2={status:ns};if(upd.start_date)dbU2.start_date=upd.start_date;if(upd.end_date)dbU2.end_date=upd.end_date;if(upd.credit_date)dbU2.credit_date=upd.credit_date;await supa.from("requests").update(dbU2).eq("id",r.id);}catch(e2){console.error(e2);}}}} style={{padding:"8px 14px",borderRadius:6,border:"1px solid #DDD",fontSize:"0.82rem",fontWeight:700,background:st.bg,color:st.c}}>
...
{["ID","Πελάτης","ΑΦΜ","Πάροχος","Κατάσταση","Agent","Καταχ.","Έναρξη","Λήξη"].map(h=><th key={h} style={{padding:"7px 8px",textAlign:"left"}}>{h}</th>)}</tr></thead>
